class Dish < ActiveRecord::Base
  ...
  def top_review
    self.reviews.select("reviews.comment_source_url, reviews.comment_source_type, reviews.remote_photo_source_type, reviews.remote_photo_source_url, reviews.remote_photo, reviews.photo, comment, user_id, users.remote_photo as user_photo, users.name as user_name").order('photo DESC').where("reviews.remote_photo IS NOT NULL || reviews.comment IS NOT NULL").limit(1).joins(:user)
  end
